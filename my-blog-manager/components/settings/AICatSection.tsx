...
import { Save, Bot, Sparkles, Sliders, MessageSquareText, Cpu, Globe, Key } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="group">
          <label className="flex items-center gap-2 text-sm font-black text-slate-700 dark:text-slate-300 mb-3">
            <Globe size={16} className="text-slate-400 group-focus-within:text-indigo-500 transition-colors" /> API 地址 (API URL)
          </label>
          <input
            type="text"
            value={config.apiUrl}
            onChange={(e) => updateConfig('apiUrl', e.target.value)}
            className="w-full bg-white/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl py-3.5 px-5 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 font-medium font-mono text-sm"
            placeholder="https://generativelanguage.googleapis.com"
          />
          <p className="text-[11px] text-slate-400 mt-2 ml-1">支持任意兼容 Google Gemini API 的反代地址，末尾不要加斜杠。</p>
        </div>

        {/* API Key */}
        <div className="group">
          <label className="flex items-center gap-2 text-sm font-black text-slate-700 dark:text-slate-300 mb-3">
            <Key size={16} className="text-slate-400 group-focus-within:text-indigo-500 transition-colors" /> API 密钥 (API Key)
          </label>
          <input
            type="password"
            value={config.apiKey}
            ...
            placeholder="AIzaSy..."
          />
          <p className="text-[11px] text-slate-400 mt-2 ml-1">留空则使用环境变量 GEMINI_API_KEY（通过 wrangler secret 设置）。</p>
        </div>
