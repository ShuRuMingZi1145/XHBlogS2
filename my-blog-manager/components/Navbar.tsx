...
export default function Navbar() {
  ...
  const handleUpdateLocal = async () => {
      ...
      try {
        ...
        const configRes = await fetch(`/backend_config.json?t=${Date.now()}`, { signal: controller.signal });
        ...
        if (error.name === 'AbortError') {
          showToast("⏱️ 请求超时，请确认 Python 后端正在运行", "error");
        } else {
                ...
                {isOpBoxOpen && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 mt-3 w-80 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-4 z-50 cursor-default">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button onClick={handleUpdateLocal} disabled={isUpdating} className={`py-2.5 rounded-xl text-xs font-black shadow-lg transition-colors ${isUpdating ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-indigo-500 text-white shadow-indigo-500/30 hover:bg-indigo-600'}`}>
                        {isUpdating ? '⏳ 处理中...' : '🚀 更新本地'}
                      </button>
                    </div>
                  </motion.div>
